...
class AuthController extends BaseController {
  ...
  async register ({ request, response, auth }) {
    const user = new User(request.only(['name', 'email', 'password', 'company']))
    ...
    const data = await auth.withRefreshToken().generate(user)
    data.user = user
  ...
  /**
    ...
    *
    */
  async loginAccountKit ({ request, response, auth }) {
    const tokenKit = request.input('token')
    await this.validate(request.all(), { token: 'required' })
    // Attempt to get fb account
    const facebookResponse = await apisauce.create({ baseURL: 'https://graph.accountkit.com/v1.3' })
      .get('/me', { access_token: tokenKit })

    if (facebookResponse.ok) {
      const facebookUser = facebookResponse.data
      let user = await User.findBy({ facebook_id: facebookUser.id })
      if (!user) {
        user = await User.create({ facebook_id: facebookUser.id, phone: facebookUser.phone })
      }
      console.log(user)
      let data = await auth.withRefreshToken().generate(user)
      data.user = user
      return response.apiSuccess(data)
    } else {
